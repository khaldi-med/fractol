...
int	handle_mouse_move(int x, int y, t_fractol *fractol)
{
	if (fractol->fractal_type == JULIA && fractol->julia_mouse_track)
	{
		...
		double real_range = 1.5;
		double imag_range = 1.5;
		...
		if (fractol->julia_c.real > 0.7) fractol->julia_c.real = 0.7;
		if (fractol->julia_c.real < -1.5) fractol->julia_c.real = -1.5;
		if (fractol->julia_c.imag > 1.0) fractol->julia_c.imag = 1.0;
		if (fractol->julia_c.imag < -1.0) fractol->julia_c.imag = -1.0;
		...
		t_fractol *fractol)
{
	...
	double mouse_real_offset = mouse_pos.real - fractol->move_x;
	double mouse_imag_offset = mouse_pos.imag - fractol->move_y;
