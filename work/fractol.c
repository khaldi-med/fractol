...
void	set_default_params(t_fractol *fractol)
{
	...
	if (fractol->fractal_type == MANDELBROT)
	{
		...
		double height_range = fractol->max.imag - fractol->min.imag;
		double width_range = height_range * aspect_ratio;
		
		/* Center the view horizontally */
		double mid_real = (fractol->min.real + fractol->max.real) / 2.0;
	...
	{
		/* 
		 * For Julia set, we don't need to set min/max values explicitly
		 * since they're calculated in pixel_to_complex based on zoom level
		 * The coordinate system is centered at (0,0) with range determined by zoom
		 */
	}
